...
export default function OrderManagement() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const q = query(collection(db, "whatsappOrders"), orderBy("createdAt", "desc"));
        ...
        const ordersList = querySnapshot.docs.map((doc) => {
          ...
          return {
            ...
            date: data.createdAt ? new Date(data.createdAt.seconds * 1000).toLocaleString('en-IN', {
              dateStyle: 'short',
              timeStyle: 'short'
            }) : "N/A",
            customer: data.customer?.name || "WhatsApp Customer",
            total: data.total || 0,
            paymentStatus: data.status === "WhatsApp Pending" ? "Payment Pending" :
              data.status === "Confirmed" ? "Success" : "Cancel",
            fulfillmentStatus: data.status === "Confirmed" ? "Fulfilled" : "Unfulfilled",
            items: data.quantity || 1,
            deliveryMethod: "WhatsApp Order",
            productTitle: data.product?.title || "N/A",
            size: data.product?.size || "N/A",
            thickness: data.product?.thickness || "N/A"
          };
        });
  ...
  // summary cards - calculate from real orders
  const summary = useMemo(() => {
    const totalOrders = orders.length;
    const orderedItems = orders.reduce((sum, order) => sum + (order.items || 0), 0);
    const fulfilled = orders.filter(o => o.fulfillmentStatus === "Fulfilled").length;
    const returns = 0; // Not implemented yet
    const delivered = fulfilled; // Assuming fulfilled = delivered
            ...
            <div className="flex items-center space-x-1">
              ...
              {[1, 2, 3, 4].map((pageNum) => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-lg transition ${pageNum === 1
